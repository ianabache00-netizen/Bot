# ============================================================
#  ZENTRIX GENERATOR BOT — Render-ready edition
#  Health check server included (Render web service)
# ============================================================
...
TIERS = {
    "Premium":  3000,   # lines per generate
    "Platinum": 2500,   # lines per generate
}
...
def reg_user(user_id, username):
    if not get_user(user_id):
        cur.execute(
            "INSERT INTO users(user_id,username,tier,gens,joined) VALUES(?,?,'None',0,?)",
            (user_id, username or "user", datetime.now().strftime("%Y-%m-%d %H:%M"))
        )
        db.commit()
...
async def do_redeem(update, ctx, key):
    ...
    cur.execute("UPDATE users SET tier=?, key_used=? WHERE user_id=?",
                (row["tier"], key, uid))
    db.commit()
...
async def stock_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    files = stock_files()
    if not files:
        return await update.message.reply_text("📦 𝙎𝙩𝙤𝙘𝙠 𝙞𝙨 𝙚𝙢𝙥𝙩𝙮.", reply_markup=back_home())
    msg = "📦 𝘼𝙫𝙖𝙞𝙡𝙖𝙗𝙡𝙚 𝙎𝙩𝙤𝙘𝙠:\n\n"
    for f in files:
        rem, total = stock_remaining(f)
        msg += f"• {f}\n  {rem}/{total} lines remaining\n"
...
# ---------------- GENERATION ----------------
async def gen_start(update, ctx, uid, msg):
    tier = user_tier(uid)
    ...
    kb = [[InlineKeyboardButton(f"📄 {f}", callback_data=f"pick:{f}")] for f in files]
    kb.append([InlineKeyboardButton("⬅️ Back", callback_data="home")])
    await msg.reply_text("🎰 𝙎𝙚𝙡𝙚𝙘𝙩 𝙨𝙩𝙤𝙘𝙠:", reply_markup=InlineKeyboardMarkup(kb))
...
async def main_async():
    # health server in background thread (keeps Render happy)
